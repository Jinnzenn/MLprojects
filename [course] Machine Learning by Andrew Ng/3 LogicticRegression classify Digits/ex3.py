...
data = scio.loadmat('ex3data1.mat')  # scipy.io库主要就是为了python编程中I/O服务的，读取训练集 包括两部分 输入特征和标签
...
# 随机抽取100个训练样本 进行可视化
rand_indices = np.random.permutation(range(m))  # 获取0-4999 5000个无序随机索引，类似指针
...
'''第2-1部分  编写逻辑回归的代价函数(正则化),做简单的测试'''  # 证明各函数功能正常运行

# 逻辑回归（正则化）代价函数的测试用例
print('Testing lrCostFunction()')

theta_t = np.array([-2, -1, 1, 2])  # 初始化假设函数的参数  假设有4个参数
X_t = np.c_[np.ones(5), np.arange(1, 16).reshape((3, 5)).T/10]  # numpy.arange()y用于创建等差数组，输入特征矩阵 5个训练样本 每个样本3个输入特征，前面添加一列特征=1
...
np.set_printoptions(formatter={'float': '{: 0.6f}'.format})  #np.set_printoptions(formatter = {'float':'{:0.6f}.format}
