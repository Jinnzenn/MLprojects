...
def plot_decision_boundary(theta, X, y):
    ...
    if X.shape[1] <= 3:  # shape[1]表示特征数
        # Only need two points to define a line, so choose two endpoints
        plot_x = np.array([np.min(X[:, 1]) - 2, np.max(X[:, 1]) + 2])  # 为什么要-2和+2？

        # Calculate the decision boundary line
        plot_y = (-1/theta[2]) * (theta[1]*plot_x + theta[0])  # 直线的两点公式 plot_y为exam2score plot_x为exam1score
