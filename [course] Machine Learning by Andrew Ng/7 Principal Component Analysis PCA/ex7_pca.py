...
# 对比两个不同的特征向量 U[:,0]更好 投影误差最小  U中的各个特征向量（列）都是正交的  2D->1D 取前1个特征向量 作为 Ureduce
rk.draw_line(mu, mu + 1.5 * S[0] * U[:, 0])   # 计算特征向量投影误差
rk.draw_line(mu, mu + 1.5 * S[1] * U[:, 1])

print('Top eigenvector: \nU[:, 0] = {}'.format(U[:, 0]))  # 利用PCA得到的特征向量矩阵 Ureduce（降维后子空间的基）
