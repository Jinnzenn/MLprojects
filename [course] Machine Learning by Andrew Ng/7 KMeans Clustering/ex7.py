...
print(len(X))
k = 3  # 随机初始化3个聚类中心
initial_centroids = np.array([[3, 3], [6, 2], [8, 5]])  # 随机设定三个聚类中心

#找到离每个样本最近的初始聚类中心序号
idx = fc.find_closest_centroids(X, initial_centroids)  # find_closest_centroids计算样本点与每个聚类中心的距离，取最小值
...
centroids = cc.compute_centroids(X, idx, k) #在簇分配结束后 对每个簇的样本点重新计算聚类中心，聚类中心的位置等于前述被分配到该聚类中心的所有样本点的均值
